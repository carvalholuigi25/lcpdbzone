...
import { AnimesModel, AuthResponse } from '@/app/models';
...
@Component({
  ...
  templateUrl: './create-animes.html',
  styleUrl: './create-animes.scss',
})
export class CreateAnimes {
  formCreateAnimes = new FormGroup({
  ...
  constructor(@Inject(DOCUMENT) private document: Document, private authService: AuthService, private animesDataService: AnimesDataService) {
  ...
  onSubmit() {
    ...
    if(promptcre !== null && promptcre == true) {
      try {
        const {title, description, studio, image, artwork, isFeatured, releaseDate, scoreRating, genre, format} = this.formCreateAnimes.value;
        const objdata: AnimesModel = {
          animeId: 0,
          title: ""+title,
        ...
        console.log(objdata)
        this.animesDataService.createAnimes(objdata).subscribe((r) => {
          alert(`Created info for animes ${r.title}!`);
